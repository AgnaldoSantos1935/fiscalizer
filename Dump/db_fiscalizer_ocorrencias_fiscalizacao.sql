...
CREATE TABLE `ocorrencias_fiscalizacao` (
  ...
  `data_ocorrencia` date DEFAULT curdate(),
  `tipo` enum('advertencia','glosa','nao_conformidade','outros') DEFAULT 'outros',
  `descricao` text DEFAULT NULL,
  `responsavel_id` int(11) DEFAULT NULL,
  PRIMARY KEY (`id_ocorrencia`),
  KEY `responsavel_id` (`responsavel_id`),
  KEY `idx_ocorrencia_contrato` (`contrato_id`),
  CONSTRAINT `ocorrencias_fiscalizacao_ibfk_1` FOREIGN KEY (`contrato_id`) REFERENCES `contratos` (`id_contrato`),
  CONSTRAINT `ocorrencias_fiscalizacao_ibfk_2` FOREIGN KEY (`responsavel_id`) REFERENCES `usuarios` (`id_usuario`)
