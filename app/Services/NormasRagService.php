...
class NormasRagService
{
    public function buscarFundamentacao(string $consulta, int $topK = 5, array $filtros = []): array
    {
        ...
        if (! empty($filtros['fonte'])) {
            $query->where('fonte', $filtros['fonte']);
        }
        if (! empty($filtros['idioma'])) {
            $query->where('idioma', $filtros['idioma']);
        }
        if (! empty($filtros['tags'])) {
            $tags = is_array($filtros['tags']) ? $filtros['tags'] : [$filtros['tags']];
            foreach ($tags as $tg) {
                $query->whereJsonContains('tags', $tg);
            }
        }
    ...
    {
        if (empty($trechos)) {
            return 'Sem fundamentações indexadas para esta consulta.';
        }
