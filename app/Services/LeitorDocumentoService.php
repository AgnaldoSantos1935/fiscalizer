...
public function identificarArquivo(UploadedFile $file)
{
    ...
    if ($ext === 'xlsx' || $ext === 'xls') return 'planilha_medicao';

    if ($ext === 'pdf') {
        $texto = Pdf::getText($file->getRealPath());

        if (strpos($texto, 'Nota Fiscal Eletrônica') !== false) return 'nfe_pdf';
        if (strpos($texto, 'NFS-e') !== false) return 'nfse_pdf';
        if (strpos($texto, 'Certidão') !== false) return 'certidao';
        if (strpos($texto, 'Relatório') !== false) return 'relatorio_execucao';
    }
