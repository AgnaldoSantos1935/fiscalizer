        ...
        for ($offset = 0; $offset < $payloadCount; $offset += $chunkSize) {
            ...
            foreach ($batch as $idx => $data) {
                ...
                if (! isset($norm['vigencia'])) {
                    $vg = [];
                    if (isset($norm['data_inicio'])) {
                        $vg['data_inicio'] = $norm['data_inicio'];
                    }
                    if (isset($norm['data_fim'])) {
                        $vg['data_fim'] = $norm['data_fim'];
                    }
                    if (! empty($vg)) {
                        $norm['vigencia'] = $vg;
                    }
                ...
                if (! isset($norm['contratada'])) {
                    $ct = [];
                    if (isset($norm['contratada_id'])) {
                        $ct['empresa_id'] = $norm['contratada_id'];
                    }
                    if (isset($norm['cnpj'])) {
                        $ct['cnpj'] = $norm['cnpj'];
                    }
                    if (isset($norm['razao_social'])) {
                        $ct['razao_social'] = $norm['razao_social'];
                    }
                    if (! empty($ct)) {
                        $norm['contratada'] = $ct;
                    }
                ...
                if (! empty($norm['itens']) && is_array($norm['itens'])) {
                    $norm['itens'] = array_map(function ($item) {
                        if (is_array($item)) {
                            if (! isset($item['descricao']) && isset($item['descricao_item'])) {
                                $item['descricao'] = $item['descricao_item'];
                            }
                            if (! isset($item['unidade']) && isset($item['unidade_medida'])) {
                                $item['unidade'] = $item['unidade_medida'];
                            }
                        }
                        ...
                        if (! $empresaId) {
                            ...
                            if (! $empresaId && ! empty($contratada['razao_social'])) {
                                ...
                                if ($enderecoData) {
                                    ...
                                    if ($p5 !== '') {
                                        $enderecoString = $enderecoString ? ($enderecoString . ' - CEP ' . $p5) : ('CEP ' . $p5);
                                    }
                                }
